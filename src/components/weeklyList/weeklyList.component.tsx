...
import { WeeklyType } from "../shared-types";
...
const WeeklyList = () => {
  const [weeklies, setWeeklies] = useState<WeeklyType[]>([]);
    ...
    const getWeeklies = async () => {
      ...
      setWeeklies(
        data.docs.map((doc) => ({
          ...doc.data(),
          id: doc.id,
          weeklyName: doc.data().weeklyName,
  ...
  const sortBossBasedOnPrice = [...filteredWeeklies].sort(
    (a, b) => a.weeklyPrice - b.weeklyPrice
  );
  ...
  const currentCharacters = sortCharacterBasedOnLvl.slice(
    firePostIndex,
    lastPostIndex
  );
